...
const PhotoToPdfTool: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    const moveImage = (fromIndex: number, toIndex: number) => {
        ...
        const newImages = [...images];
        const itemToMove = newImages[fromIndex];
        newImages.splice(fromIndex, 1);
        newImages.splice(toIndex, 0, itemToMove);
        setImages(newImages);
    };

    // --- 1. LOGIKA DESKTOP (HTML5 DRAG & DROP) ---
    ...
    const handleTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
        ...
        if (closestItem) {
            ...
            const gridContainer = targetElement?.closest('#grid-container');
            if (gridContainer) {
                moveImage(draggedItemIndex, images.length - 1);
            }
        }
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            pdf.save(finalName);
        } catch (error) { console.error(error); alert("Gagal convert PDF."); } 
        ...
        <div className="animate-in fade-in zoom-in duration-300">
                                    ...
                                    {images.map((img, idx) => {
                                        ...
                                        return (
                                            <div 
                                                ...
                                                className={`relative flex flex-col gap-2 group cursor-grab active:cursor-grabbing touch-manipulation ${draggedItemIndex === idx ? 'opacity-40 scale-95 ring-2 ring-yellow-400 rounded-lg' : 'opacity-100'}`}
                                            >
                                                <div className={`rounded-lg border transition-all duration-500 ease-in-out pointer-events-none ${isFitMode ? 'bg-transparent border-gray-700 p-0' : 'bg-gray-800/50 border-gray-700/50 p-2 md:p-4'}`}>
                    ...
                    <div className="w-full lg:w-80 lg:sticky lg:top-24 space-y-6">
                        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-2xl">
                            <h3 className="text-white font-bold mb-6 flex items-center gap-2 pb-4 border-b border-gray-800">
                                <Settings2 size={20} className="text-yellow-400"/> Pengaturan
                            </h3>
                            <div className="mb-6">
            ...
            {editIndex !== null && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black p-0 md:p-4">
                    <div className="w-full h-full md:h-[85vh] max-w-5xl bg-gray-900 border-none md:border border-gray-700 md:rounded-2xl flex flex-col shadow-2xl">
                        <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-gray-900 md:rounded-t-2xl shrink-0">
                        </div>
                        <div className="flex-1 bg-black overflow-hidden relative flex items-center justify-center">
                            <Cropper ref={cropperRef} src={images[editIndex]} style={{ height: "100%", width: "100%" }} initialAspectRatio={0} guides={true} viewMode={1} dragMode="move" background={false} responsive={true} autoCropArea={0.9} checkOrientation={false} />
                        </div>
                        <div className="p-4 border-t border-gray-800 bg-gray-900 flex justify-between md:justify-end gap-3 md:rounded-b-2xl shrink-0 safe-area-bottom">
                            <button onClick={() => setEditIndex(null)} className="flex-1 md:flex-none px-4 py-3 bg-gray-800 text-white rounded-lg font-bold text-sm">Batal</button>
                            <button onClick={handleCropSave} className="flex-[2] md:flex-none px-6 py-3 bg-yellow-400 text-black rounded-lg font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform text-sm"><Check size={18}/> Simpan</button>
                        </div>
                    </div>
                </div>
            )}
...
// ============================================================================
// CONSTANTS & UTILS (SHARED PRECISE MATH)
// ============================================================================
const THEME_COLORS = {
    keyword: [197, 134, 192], 
...
// PRECISE MEASUREMENTS (IN POINTS)
const PDF_CONFIG = {
    MARGIN: 30,           
...
`# Tata cara menggunakan tools ini
# Kalian bisa ketik manual atau import codenya
# Inputnya masukkin di kolom yang disediakan
# Inputnya sesuai urutan
# Sisanya eksplor sendiri
# Disclaimer: saya tidak bertanggung jawab apabila code anda hilang karena pindah page
# Disclaimer: saya tidak bertanggung jawab apabila api python offline secata tiba-tiba
# Disclaimer: saya tidak bertanggung jawab apabila kalian sedang ngoding, website tiba-tiba down/browser kalian force close
# Selamat mencoba :P
...
const CodeToPdfTool: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    // --- STATE ---
    const [code, setCode] = useState<string>(DEFAULT_PYTHON_CODE.replace(/\t/g, '    '));
    const [terminalLogs, setTerminalLogs] = useState<{ type: 'output' | 'user-input' | 'system'; text: string }[]>([]);
    const [isWaitingInput, setIsWaitingInput] = useState(false);
    const [userInput, setUserInput] = useState("");
    const [pythonStdin, setPythonStdin] = useState("100000\n25000\n10"); 
    const inputResolverRef = useRef<((value: string) => void) | null>(null);
    
    const textareaRef = useRef<HTMLTextAreaElement>(null);
    const highlightRef = useRef<HTMLPreElement>(null);

    const [fileName, setFileName] = useState(`CODE_${new Date().toISOString().slice(0,10)}`);
    const [fontSize, setFontSize] = useState<number>(11);
    const [language, setLanguage] = useState<'javascript' | 'python'>('python');
    const [pageSize, setPageSize] = useState<string>('a4'); 
    const [bgTheme, setBgTheme] = useState<string>('#000000'); 
...
`;
                ...
                printToTerminal("Offline / API Error.", 'output');
            }
        }
        setIsRunning(false);
    };
    ...
    const generatePDF = () => {
        ...
        try {
            ...
            const renderProcessedLines = (lines: any[], isHeader: boolean, title: string) => {
                ...
                if (cursorY + PDF_CONFIG.HEADER_HEIGHT > pageHeight - margin) {
                    pdf.addPage();
                    cursorY = margin;
                }
                    ...
                    lineObj.tokens.forEach((t: any) => {
                         if (isColored) {
                            ...
                            if (t.type === 'text' && isLight) pdf.setTextColor(0,0,0);
                             ...
                             pdf.setTextColor(isLight ? 0 : 255);
                        }
        ...
        } catch (error) { console.error(error); alert("Gagal render PDF."); } 
        finally { setIsGenerating(false); }
    };
    ...
    const PdfPreviewComponent = () => {
        ...
        const simulatePagination = () => {
            ...
            const renderSection = (lines: any[], title: string) => {
                const headerNode = (
                     <div key={`head-${title}-${Math.random()}`} style={{
                        height: `${PDF_CONFIG.HEADER_HEIGHT}pt`,
            ...
            const termLines = terminalLogs.filter(l => l.type !== 'system').map(l => l.type === 'user-input' ? `> ${l.text}` : l.text);
            if (termLines.length > 0) {
                if (currentY + PDF_CONFIG.BLOCK_GAP < PAGE_HEIGHT - MARGIN) {
                     currentPageNodes.push(<div key="gap" style={{height: `${PDF_CONFIG.BLOCK_GAP}pt`}}></div>);
                     currentY += PDF_CONFIG.BLOCK_GAP;
                }
        ...
        return (
            <div className="w-full bg-gray-500/20 p-8 overflow-auto rounded-xl flex flex-col items-center gap-8 min-h-[60vh] max-h-[80vh] custom-scrollbar">
                {pages.map((pageNodes, i) => (
                    <div 
                        key={i}
                        className="bg-white shadow-2xl relative shrink-0 transition-transform duration-300 origin-top"
                        style={{ 
                            width: `${dim.w}pt`,
                            height: `${dim.h}pt`,
                            padding: `${MARGIN}pt`,
                    ...
                    >
                        ...
                        <div className="absolute bottom-2 right-4 text-[10px] text-gray-400 font-sans">
                            Page {i + 1}
                        </div>
                    </div>
                ))}
    ...
    return (
        <div className="animate-in fade-in zoom-in duration-300 flex flex-col h-full">
            <div className="mx-auto max-w-7xl w-full mb-4 px-4 sm:px-0">
                <button onClick={onBack} className="flex items-center gap-2 text-gray-400 hover:text-yellow-400 transition-colors font-bold group">
                    <div className="p-2 rounded-full bg-gray-900 group-hover:bg-gray-800 border border-gray-800"><ArrowLeft size={20} /></div>
                    <span>Kembali</span>
                </button>
            </div>
            ...
            <div className="mx-auto max-w-7xl w-full flex flex-col lg:flex-row gap-6 px-4 sm:px-0 pb-12">
                <div className="w-full lg:flex-1 flex flex-col gap-4">
                    <div className="flex gap-2 mb-2 bg-gray-900 p-1.5 rounded-lg border border-gray-800 w-fit">
                    ...
                    {activeTab === 'editor' ? (
                        <div className="flex flex-col gap-4">
                            <div className="border border-gray-800 rounded-xl overflow-hidden shadow-2xl flex flex-col h-[50vh] relative transition-colors duration-300" style={{ backgroundColor: bgTheme }}>
                                <div className="bg-black/20 border-b border-white/10 p-2 flex justify-between items-center backdrop-blur-sm z-10 relative">
                                    <span className={`text-xs font-mono ml-2 ${isLightBackground(bgTheme) ? 'text-black' : 'text-gray-300'}`}>main.{language === 'python' ? 'py' : 'js'}</span>
                                </div>
                                ...
                                <div className="relative flex-1 w-full h-full overflow-hidden">
                                    <pre 
                                        ref={highlightRef}
                                        aria-hidden="true"
                                        className={`absolute inset-0 p-4 font-mono text-sm leading-relaxed whitespace-pre-wrap break-words pointer-events-none custom-scrollbar overflow-hidden ${isColored ? 'opacity-100' : 'opacity-0'}`}
                                    >
                                        {code.split('\n').map((line, i) => (
                                            <div key={i}>
                                    ...
                                    <textarea
                                        ...
                                        className={`absolute inset-0 w-full h-full p-4 font-mono text-sm leading-relaxed resize-none outline-none bg-transparent whitespace-pre-wrap break-words custom-scrollbar ${isColored ? `text-transparent caret-${isLightBackground(bgTheme) ? 'black' : 'white'}` : (isLightBackground(bgTheme) ? 'text-black' : 'text-[#d4d4d4]')}`}
                                        spellCheck={false}
                                    />
                                </div>

                                <button onClick={runCode} disabled={isRunning} className="absolute bottom-6 right-6 bg-green-600 hover:bg-green-500 text-white px-5 py-3 rounded-full shadow-lg font-bold z-10 flex items-center gap-2">
                            ...
                            {language === 'python' && (
                                <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
                                    <label className="block text-xs font-bold text-gray-400 mb-2 flex justify-between">
                                        <span>Input</span>
                                        <span className="text-[10px] text-gray-500">Ketik input per baris</span>
                                    </label>
                                    <textarea 
                                        value={pythonStdin}
                                        onChange={(e) => setPythonStdin(e.target.value)}
                                        className="w-full bg-black border border-gray-800 rounded-lg p-3 text-gray-300 font-mono text-xs h-20 outline-none focus:border-blue-500"
                                        placeholder="Contoh:&#10;1&#10;2"
                                    />
                                </div>
                            )}

                            <div className="border border-gray-800 rounded-xl overflow-hidden shadow-xl flex flex-col min-h-[25vh] max-h-[40vh] transition-colors duration-300" style={{ backgroundColor: bgTheme }}>
                                <div className="bg-black/20 px-3 py-1.5 flex justify-between items-center border-b border-white/10">
                                    <span className={`text-[10px] font-bold ${isLightBackground(bgTheme) ? 'text-green-800' : 'text-green-500'}`}>Terminal</span>
                                    <button onClick={clearTerminal} className="text-gray-500 hover:text-red-500"><Trash2 size={12}/></button>
                                </div>
                                <div className="flex-1 p-4 overflow-y-auto font-mono text-xs custom-scrollbar">
                                    {terminalLogs.map((log, idx) => (
                                        <div key={idx} className={`mb-1 whitespace-pre-wrap break-words ${
                                            ...
                                            log.type === 'user-input' ? (isLightBackground(bgTheme) ? 'text-blue-700 font-bold' : 'text-cyan-400 font-bold') :
                                            (isLightBackground(bgTheme) ? 'text-green-800' : 'text-green-400')
                                        }`}>
                                            {log.type === 'user-input' && <span className="mr-2 opacity-50">&gt;</span>}{log.text}
                                        </div>
                                    ))}
                                    {isWaitingInput && (
                                        <div className="flex items-center">
                                            <span className={`mr-2 font-bold ${isLightBackground(bgTheme) ? 'text-blue-700' : 'text-cyan-400'}`}>&gt;</span>
                                            <input 
                                                autoFocus
                                                type="text" 
                                                value={userInput}
                                                ...
                                                className={`flex-1 bg-transparent border-none outline-none font-mono ${isLightBackground(bgTheme) ? 'text-blue-700' : 'text-cyan-400'}`}
                                            />
                                        </div>
                                    )}
                                </div>
                ...
                <div className="w-full lg:w-72 space-y-6">
                    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 shadow-xl sticky top-24">
                        ...
                        <div className="space-y-5">
                            <label className="block w-full cursor-pointer bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded-lg p-3 text-center transition-all group">
                                <div className="flex flex-col items-center gap-1">
                                    <Upload size={20} className="text-gray-400 group-hover:text-yellow-400"/>
                                    <span className="text-xs font-bold text-gray-300">Upload Code</span>
                                </div>
                                <input type="file" className="hidden" onChange={handleFileUpload} />
                            </label>

                            <div>
                                <label className="block text-[10px] font-bold text-gray-500 uppercase mb-2">Nama File</label>
                                <div className="relative">
                                    <input type="text" value={fileName} onChange={(e) => setFileName(e.target.value)} className="w-full bg-black border border-gray-700 rounded p-2 text-white text-xs outline-none focus:border-yellow-400" placeholder="Nama file..."/>
                                    <Pencil size={12} className="absolute right-2 top-2.5 text-gray-500"/>
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="block text-[10px] font-bold text-gray-500 uppercase mb-2">Background</label>
                                <div className="flex items-center gap-2 bg-black p-2 rounded border border-gray-700">
                                    <input type="color" value={bgTheme} onChange={(e) => setBgTheme(e.target.value)} className="w-8 h-8 rounded cursor-pointer border-none bg-transparent"/>
                                    <span className="text-xs text-gray-300 font-mono">{bgTheme}</span>
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="block text-[10px] font-bold text-gray-500 uppercase mb-2">Warna</label>
                                <div className="flex bg-black rounded p-1 border border-gray-700">
                            ...
                            <button 
                                ...
                                disabled={!code.trim()}
                                className="w-full py-2.5 rounded-lg font-bold text-xs bg-yellow-400 text-black hover:bg-yellow-300 border border-yellow-400 hover:shadow-lg transition-all flex items-center justify-center gap-2 mt-4"
                            >
                                <FileCode size={16} /> Download .{language === 'python' ? 'py' : 'js'}
                            </button>
...
const PlaceholderTool: React.FC<{ title: string; onBack: () => void }> = ({ title, onBack }) => (
    <div className="animate-in fade-in zoom-in duration-300 flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <div className="bg-gray-900/50 p-12 rounded-3xl border border-gray-800 flex flex-col items-center max-w-lg mx-auto">
            <div className="w-20 h-20 bg-gray-800 rounded-full flex items-center justify-center mb-6 text-yellow-400">
                <Construction size={48} />
            </div>
            <h2 className="text-3xl font-bold text-white mb-2">{title}</h2>
            <p className="text-gray-400">Belom jadi, balik lagi nanti.</p>
        </div>
    </div>
);

// ============================================================================
// 3. MAIN COMPONENT: MENU HUB
// ============================================================================
                ...
                <div className="mx-auto max-w-7xl text-center">
                    ...
                    <div className="text-center">
                        <div className="flex justify-center items-center gap-4 mb-4">
                            ...
                            <h1 className="text-4xl font-bold tracking-wider uppercase text-white sm:text-5xl">PDF Tools</h1>
                        </div>
                    </div>
                    ...
                    <div className="mx-auto mt-16 max-w-5xl">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <button onClick={() => setActiveTool('photo')} className="group relative bg-gray-900 border border-gray-800 hover:border-yellow-400 rounded-2xl p-8 text-left transition-all duration-300 hover:shadow-lg hover:shadow-yellow-400/10 hover:-translate-y-2 flex items-center gap-6">
                                <div className="p-4 bg-gray-800 rounded-xl text-yellow-400 group-hover:bg-yellow-400 group-hover:text-black transition-colors">
                                    <ImagePlus size={32} />
                                </div>
                                <div>
                            ...
                            <button onClick={() => setActiveTool('code')} className="group relative bg-gray-900 border border-gray-800 hover:border-blue-500 rounded-2xl p-8 text-left transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/10 hover:-translate-y-2 flex items-center gap-6">
                                <div className="p-4 bg-gray-800 rounded-xl text-blue-500 group-hover:bg-blue-500 group-hover:text-white transition-colors">
                                    <FileCode size={32} />
                                </div>
                                <div>
                            ...
                            <button onClick={() => setActiveTool('merge')} className="group relative bg-gray-900 border border-gray-800 hover:border-green-500 rounded-2xl p-8 text-left transition-all duration-300 hover:shadow-lg hover:shadow-green-500/10 hover:-translate-y-2 flex items-center gap-6">
                                <div className="p-4 bg-gray-800 rounded-xl text-green-500 group-hover:bg-green-500 group-hover:text-white transition-colors">
                                    <Files size={32} />
                                </div>
                                <div>
                            ...
                            <button onClick={() => setActiveTool('edit')} className="group relative bg-gray-900 border border-gray-800 hover:border-purple-500 rounded-2xl p-8 text-left transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/10 hover:-translate-y-2 flex items-center gap-6">
                                <div className="p-4 bg-gray-800 rounded-xl text-purple-500 group-hover:bg-purple-500 group-hover:text-white transition-colors">
                                    <FilePenLine size={32} />
                                </div>
                                <div>
